...
class QRouteDouble(QRoute):
    def make_elements(self, pts: np.ndarray):
        ...
        self.add_qgeometry('path', {'trace': line},
                           width=2*p.trace_width + p.trace_gap,
                           fillet=p.fillet,
                           layer=p.layer)
...
##In all that follows, only the object heritage was changed (from QRoute to QRouteDouble)
class CPMutual(QRouteDouble):
