...
class circuFlatDoubleCell(QComponent):
    ...
    def make_cell(self, offset_x=0, offset_y=0, affix=None, fully_round=None):
        ...
        bottom_central_pts = np.concatenate((top_central_pts[4:24], [top_central_pts[4]])) - [0, offset_top_bottom]
        ...
        #dirty overwrite
        if fully_round=="left":
            top_diel = draw_rectangle_corner_offset(p.cell_length*2 + 5*p.alignment_tol, p.jj_wire_to_ground_gap + 2*p.alignment_tol + p.ground_cap_width,
                                                    -5*p.alignment_tol, wire_center + p.jj_lead_width/2 + p.jj_wire_to_ground_gap/2)
            bottom_diel = draw_rectangle_corner_offset(p.cell_length*2 + 5*p.alignment_tol, -(p.jj_wire_to_ground_gap + 2*p.alignment_tol + p.ground_cap_width),
                                                    -5*p.alignment_tol, -(wire_center + p.jj_lead_width/2 + p.jj_wire_to_ground_gap/2))
            central_diel = draw.rectangle(p.cell_length*2 + 5*p.alignment_tol, p.central_width + p.jj_wire_to_cap_gap,
                                          p.cell_length - 2.5*p.alignment_tol, 0)
        ...
        second_bottom_central_pts = np.array([
            ...
            [p.cell_length + p.cap_lead_width/2, -wire_center + p.jj_lead_width/2 - p.alignment_tol],
        ...
        if fully_round=="right":
            ...
            sbr_middle = p.cell_length*2 - p.top_cap_overlap_pad/2 + (p.top_cap_overlap_pad/2 - 2*p.extremity_gap)/2
            second_bottom_right = round_polygon(draw.Polygon([
                ...
                [sbr_middle + p.cap_lead_width/2*3/4, -wire_center + p.jj_lead_width/2 - p.alignment_tol],
                [sbr_middle + p.cap_lead_width/2*3/4, -wire_center - p.jj_lead_width/2 - p.jj_wire_to_ground_gap - p.alignment_tol - p.ground_cap_width],
                ...
                [sbr_middle - p.cap_lead_width/2*3/4, -wire_center - p.jj_lead_width/2 - p.jj_wire_to_ground_gap - p.alignment_tol],
                [sbr_middle - p.cap_lead_width/2*3/4, -wire_center - p.jj_lead_width/2 + p.alignment_tol],
                [p.cell_length*2 - p.top_cap_overlap_pad/2, -wire_center - p.jj_lead_width/2 + p.alignment_tol],
                [p.cell_length*2 - p.top_cap_overlap_pad/2, -wire_center + p.jj_lead_width/2 - p.alignment_tol]                
            ]), p.corner_rounding)
            second_top_right = round_polygon(draw.Polygon([
                ...
                [p.cell_length*3/2 + p.cap_to_cap_dist/2 + p.top_cap_reduction, +p.central_width/2 - p.alignment_tol],
                [sbr_middle - p.cap_lead_width/2*3/4, +p.central_width/2 - p.alignment_tol],
                [sbr_middle - p.cap_lead_width/2*3/4, +wire_center - p.jj_lead_width/2 + p.alignment_tol],
                [p.cell_length*2 - p.top_cap_overlap_pad/2, +wire_center - p.jj_lead_width/2 + p.alignment_tol],
                [p.cell_length*2 - p.top_cap_overlap_pad/2, +wire_center + p.jj_lead_width/2 - p.alignment_tol],
                [sbr_middle - p.cap_lead_width/2*3/4, +wire_center + p.jj_lead_width/2 - p.alignment_tol],
                ...
                [sbr_middle + p.cap_lead_width/2*3/4, +wire_center + p.jj_lead_width/2 + p.jj_wire_to_ground_gap + p.ground_cap_width + p.alignment_tol],
                [sbr_middle + p.cap_lead_width/2*3/4, -p.central_width/2 + p.alignment_tol - p.outer_intercap_extra_l]
            ]), p.corner_rounding)
        ...
        if fully_round=="left":
            ...
            sbl_middle = p.top_cap_overlap_pad/2 - (p.top_cap_overlap_pad/2 - 2*p.extremity_gap)/2
            second_bottom_left = round_polygon(draw.Polygon([
                [p.top_cap_overlap_pad/2, -wire_center + p.jj_lead_width/2 - p.alignment_tol],
                [sbl_middle - p.cap_lead_width/2*3/4, -wire_center + p.jj_lead_width/2 - p.alignment_tol],
                [sbl_middle - p.cap_lead_width/2*3/4, -wire_center - p.jj_lead_width/2 - p.jj_wire_to_ground_gap - p.alignment_tol],
                [sbl_middle - p.cap_lead_width/2*3/4, -wire_center - p.jj_lead_width/2 - p.jj_wire_to_ground_gap - p.alignment_tol - p.ground_cap_width],
            ...
            second_top_left = round_polygon(draw.Polygon([
                ...
                [p.cell_length/2 - p.cap_to_cap_dist/2 - p.top_cap_reduction + p.outer_intercap_extra_l, +p.central_width/2 - p.alignment_tol],
                [sbl_middle + p.cap_lead_width/2*3/4, +p.central_width/2 - p.alignment_tol],
                [sbl_middle + p.cap_lead_width/2*3/4, +wire_center - p.jj_lead_width/2 + p.alignment_tol],
                [p.top_cap_overlap_pad/2, +wire_center - p.jj_lead_width/2 + p.alignment_tol],
                [p.top_cap_overlap_pad/2, +wire_center + p.jj_lead_width/2 - p.alignment_tol],
                [sbl_middle + p.cap_lead_width/2*3/4, +wire_center + p.jj_lead_width/2 - p.alignment_tol],
                ...
                [sbl_middle - p.cap_lead_width/2*3/4, +wire_center + p.jj_lead_width/2 + p.jj_wire_to_ground_gap  + p.ground_cap_width + p.alignment_tol],
                [sbl_middle - p.cap_lead_width/2*3/4, -p.central_width/2 + p.alignment_tol]
            ]), p.corner_rounding)
            
        #junctions, all manual
        
        jj_length = p.jj_to_bridge_length + 2*p.jj_t_size + p.alignment_tol
