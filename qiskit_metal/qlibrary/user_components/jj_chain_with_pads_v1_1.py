...
class JJ_chain_with_pads(QComponent):
    ...
    def make_cell(self, offset_x=0, offset_y=0, affix=None, fully_round=None):
        ...
        lead_length = p.jj_to_bridge_length - p.jj_fine_length
        pad1 = draw.union(draw.rectangle(wire_length, p.jj_lead_width, -p.jj_to_jj_dist/2 + wire_length/2),
                          draw.rectangle(lead_length, p.jj_bridge_length, -p.jj_bridge_width/2 - p.jj_to_bridge_length + lead_length/2))
        pad2 = draw.union(draw.rectangle(wire_length, p.jj_lead_width, p.jj_to_jj_dist/2 - wire_length/2),
                          draw.rectangle(-lead_length, p.jj_bridge_length, p.jj_bridge_width/2 + p.jj_to_bridge_length - lead_length/2))
