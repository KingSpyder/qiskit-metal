...
class circuFlatDoubleCell(QComponent):
    ...
    def make_cell(self, offset_x=0, offset_y=0, affix=None, fully_round=None):
        ...
        if fully_round=="left":
            top_left = draw.union(
                top_left,
                draw.rectangle(p.panick_notch_size + p.corner_rounding, p.jj_lead_width,
                               p.panick_notch_size/2 + p.corner_rounding/2, wire_center)
            )
            ...
            top_right = draw.union(
                top_right,
                draw.rectangle(p.panick_notch_size + p.corner_rounding, p.jj_lead_width,
                               2*p.cell_length - p.panick_notch_size/2 - p.corner_rounding/2, wire_center)
            )
        ...
        if fully_round=="right":
            ...
            unround_br = draw.rectangle(p.corner_rounding, p.corner_rounding,
                                        p.cell_length*2 + p.extremity_length - p.corner_rounding/2, -unround_height)
